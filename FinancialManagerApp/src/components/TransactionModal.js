...
const TransactionModal = ({ 
  ...
  const handleSubmit = () => {
    ...
    if (type === 'income' && amountNum > bankBalance) {
      Alert.alert('දෝෂය', 'බැංකු ශේෂය ප්‍රමාණවත් නොවේ. දැනට ඇති ශේෂය: ' + formatCurrency(bankBalance));
      return;
    }
    
    if (type === 'expense' && amountNum > cashBalance) {
      Alert.alert('දෝෂය', 'මුදල් ශේෂය ප්‍රමාණවත් නොවේ. දැනට ඇති ශේෂය: ' + formatCurrency(cashBalance));
      return;
    }
              ...
              <ScrollView style={styles.categorySelector}>
                {categories.length === 0 ? (
                  <Text style={styles.emptyStateText}>
                    කාණ්ඩ නොමැත. පළමුව කාණ්ඩයක් එකතු කරන්න.
                  </Text>
                ) : (
                    ...
                    >
                      <View>
                        ...
                        <Text style={styles.categoryOptionSubtext}>
                          ඉලක්කය: {formatCurrency(category.target)} | ඉතිරි: {formatCurrency(Math.max(0, category.target - category.spent))}
                        </Text>
                      </View>
                      <Text style={styles.categoryOptionSubtext}>
